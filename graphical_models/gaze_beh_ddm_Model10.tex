%model 10
%delta varies by gaze, head orientation, and emotion
...
\resizebox{1\textwidth}{!}{
    \begin{tikzpicture}
        ...
        \node[obs, xshift=3cm](RT) {$\var{RT}_{efgij}$}; %RT grey node
        \node[obs, above=of RT,xshift= 6.5cm, yshift= -0.25cm] (C) {$\var{C}_{efgij}$}; %choice ("C") grey node
        ...
        %delta
        \node[latent, right=of tau_i, xshift= 2.8cm] (delta_i) {$\delta_{efgi}$};
        \node[latent, above=of delta_i, xshift= -0.6cm,yshift=1.7cm] (delta_mu) {$\delta_{\mu_{efgh}}$};
        \node[latent, above=of delta_i, xshift= 1cm,yshift=2cm] (delta_sigma) {$\delta_{\sigma_h}$};
        \node[latent, left=of delta_i, xshift=0.3cm,yshift=1.7cm] (delta_iprime) {$\delta_{efgi'}$};
        \factor[above=of delta_i] {delta_i-f} {right:\tiny $\phi(\delta_{\mu}_{efgh}$+${\delta}_{\sigma_h}$*${\delta}_{efgi'})$*8-4} {delta_mu, delta_sigma,delta_iprime} {delta_i} 
        ...
        %wiener distribution
        \factor[above=of RT, xshift= 2cm, yshift= 0.5cm] {C_2} {right:$\mathcal{W}(\alpha_i, 1-\beta_i, -\delta_{efgi}, \tau_i)$} {} {RT}; 
        \factor[above=of RT, xshift= -1.5cm, yshift= 0.5cm] {C_1} {left:$\mathcal{W}(\alpha_i, \beta_i, \delta_{efgi}, \tau_i)$} {} {RT};
        ...
        %for all emo cond f in 1:total conds (Q)
        \plate{emo_cond} { 
        (head_cond) 
        (gaze_cond) 
        (delta_iprime)
        (delta_mu) (delta_i)} {\tiny Emo cond $e$ =  $1,...,2$}
