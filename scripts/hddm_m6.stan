...
// absent) and emotion (neutral, fearful). threshold sep. is allowed to vary 
// within-subjects by emotion (neutral, fearful) condition.
functions{
  ...
  for (i in 1:N_obs){ 
    if (cond[i]==1){ // if signal = present
      if(choice[i]==1){ //if response = YES (correct)
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i]], sub_delta_present[subj[i],cond_other[i]]);
      } else { //if response is NO (incorrect), invert beta and delta
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i]], -sub_delta_present[subj[i],cond_other[i]]);
      }
    } else { // if signal = absent 
      if(choice[i]==1){ // if response = YES (incorrect)
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i]], sub_delta_absent[subj[i],cond_other[i]]);
      } else { // if response = NO (correct), invert beta and delta
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i]], -sub_delta_absent[subj[i],cond_other[i]]);
      }
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N_groups){
    for (j in 1:N_cond_other){
      mu_alpha_cond[i,j] = 0.1 + 3.9*Phi(mu_grp_alpha_pr[i]+cond_alpha_pr[i,j]);
  ...
  //     if(cond[i]==1){ // if signal = present
  //       if(choice[i]==1){ // if response = YES (correct)
  //         log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i]], sub_delta_present[subj[i],cond_other[i]]);
  //       } else { // if response = NO (incorrect), invert beta and delta
  //         log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i]], -sub_delta_present[subj[i],cond_other[i]]);
  //       }
  //     } else{ // if signal = absent 
  //       if(choice[i]==1){ // if response = YES (incorrect)
  //         log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i]], sub_delta_absent[subj[i],cond_other[i]]);
  //       } else { // if response = NO (correct), invert beta and delta
  //         log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i]], -sub_delta_absent[subj[i],cond_other[i]]);
  //       }
  //     }
  //   }
  // }
