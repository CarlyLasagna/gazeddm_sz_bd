%model 8 - beta and alpha also varies by emo
...
\resizebox{1\textwidth}{!}{
    \begin{tikzpicture}
        ...
        %beta
        \node[latent, left=of tau_i, xshift= -1.5cm] (beta_i) {$\beta_{fi}$};
        \node[latent, above=of beta_i, xshift= -0.6cm,yshift=2cm] (beta_mu) {$\beta_{\mu_{fh}}$};
        \node[latent, above=of beta_i, xshift= 0.9cm,yshift=2cm] (beta_sigma) {$\beta_{\sigma_h}$};
        \node[latent, left=of beta_i, xshift=0.3cm,yshift=1.7cm] (beta_iprime) {$\beta_{fi'}$};
        \factor[above=of beta_i] {beta_i-f} {right:\tiny $\phi(\beta_{\mu_fh}$+$\beta_{\sigma_h}$*$\beta_fi')$} {beta_mu, beta_sigma,beta_iprime} {beta_i}
        ...
        %alpha
        \node[latent, left=of beta_i, xshift= -2.1cm] (alpha_i) {$\alpha_{fi}$};
        \node[latent, above=of alpha_i, xshift= -0.6cm,yshift=2cm] (alpha_mu) {$\alpha_{\mu_{fh}}$};
        \node[latent, above=of alpha_i, xshift= 0.9cm,yshift=2cm] (alpha_sigma) {$\alpha_{\sigma_h}$};
        \node[latent, left=of alpha_i, xshift=0.3cm,yshift=1.7cm] (alpha_iprime) {$\alpha_{fi'}$};
        \factor[above=of alpha_i] {alpha_i-f} {right:\tiny $\phi(\alpha_{\mu_{fh}}$+$\alpha_{\sigma_h}$*$\alpha_{fi'})$*3.9+0.1} {alpha_mu, alpha_sigma,alpha_iprime} {alpha_i}  
        ...
        %wiener distribution
        \factor[above=of RT, xshift= 2cm, yshift= 0.5cm] {C_2} {right:$\mathcal{W}(\alpha_{fi}, 1-\beta_{fi}, -\delta_{fgi}, \tau_i)$} {} {RT}; 
        \factor[above=of RT, xshift= -1.5cm, yshift= 0.5cm] {C_1} {left:$\mathcal{W}(\alpha_{fi}, \beta_{fi}, \delta_{fgi}, \tau_i)$} {} {RT};
        ...
        %for all emo cond f in 1:total conds (P)
        \plate{emo_cond} { 
        (gaze_cond) 
        (delta_iprime)
        (delta_mu) (delta_i)} {\tiny Emo cond $f$ =  $1,...,2$}

        %for all emo cond f in 1:total conds (P)
        \plate{emo_cond2} {  
        (beta_iprime) (beta_iprime)
        (beta_mu) (beta_i)} {\tiny Emo cond $f$ =  $1,...,2$}

        %for all emo cond f in 1:total conds (P)
        \plate{emo_cond3} {  
        (alpha_iprime) (alpha_iprime)
        (alpha_mu) (alpha_i)} {\tiny Emo cond $f$ =  $1,...,2$}
