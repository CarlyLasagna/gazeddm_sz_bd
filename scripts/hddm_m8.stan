...
// with a non-centered parameterization. YES responses (choice=1) are modeled
// as upper boundary responses and NO responses (choice=2) are modeled as
...
// absent) and emotion (neutral, fearful). threshold sep. AND start point are 
// allowed to vary within-subjects by emotion (neutral, fearful) condition.
functions{
...
parameters { 
  ...
  matrix[N_groups,N_cond_other] cond_alpha_pr;      // condition effect
  matrix[N_groups,N_cond_other] cond_beta_pr;      // condition effect
  matrix[N_groups,N_cond_other] cond_delta_pr;      // condition effect

  //SUBJECT-level parameters
  matrix[N_subj,N_cond_other] sub_alpha_pr;                // threshold sep. subject mean, by emo conditions
  matrix[N_subj,N_cond_other] sub_beta_pr;                 // start point subject mean, by emo conditions
  matrix[N_subj,N_cond_other] sub_delta_present_pr;        // drift rate subject mean, stim present by emo conditions
  matrix[N_subj,N_cond_other] sub_delta_absent_pr;         // drift rate subject mean, stim absent by emo conditions
...
transformed parameters { 
  ...
  matrix<lower=0,upper=4>[N_subj,N_cond_other] sub_alpha;                // threshold sep. TRANSFORMED subject mean, by emo conditions
  matrix<lower=0,upper=1>[N_subj,N_cond_other] sub_beta;                 // start point TRANSFORMED subject mean, by emo conditions
  matrix<lower=-4,upper=4>[N_subj,N_cond_other] sub_delta_present;       // drift rate TRANSFORMED subject mean, stim present by emo conditions
  matrix<lower=-4,upper=4>[N_subj,N_cond_other] sub_delta_absent;        // drift rate TRANSFORMED subject mean, stim absent by emo conditions
  ...
  for (i in 1:N_subj){
   for (j in 1:N_cond_other){
     sub_alpha[i,j] = 0.1 + 3.9 * Phi(mu_grp_alpha_pr[subj_group[i]] + sig_grp_alpha_pr[subj_group[i]] * sub_alpha_pr[i,j] + cond_alpha_pr[subj_group[i],j]); //from meeting with tim -- he says to do this to add condition: "sub_alpha[subj[i]] = 0.1 + 3.9 * Phi(mu_grp_alpha_pr[group[i]] + sig_grp_alpha_pr[group[i]] * sub_alpha_pr[subj[i]]);"
...
model {
  ...
  to_vector(cond_alpha_pr) ~ normal(0, 1);
  to_vector(cond_beta_pr) ~ normal(0, 1);
  to_vector(cond_delta_pr) ~ normal(0, 1);
  
  //SUBJECT-level priors
  to_vector(sub_alpha_pr) ~ normal(0, 1);              // prior on untransformed threshold sep subj mean, by emo conditions
  to_vector(sub_beta_pr)  ~ normal(0, 1);              // prior on untransformed start point subj mean, by emo conditions
  to_vector(sub_delta_present_pr) ~ normal(0, 1);     // prior on untransformed drift rate subj mean, stim present by emo conditions
  to_vector(sub_delta_absent_pr) ~ normal(0, 1);      // prior on untransformed drift rate subj mean, stim absent by emo conditions
  ...
  for (i in 1:N_obs){ 
    if (cond[i]==1){ // if signal = present
      if(choice[i]==1){ //if response = YES (correct)
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i],cond_other[i]], sub_delta_present[subj[i],cond_other[i]]);
      } else { //if response is NO (incorrect), invert beta and delta
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i],cond_other[i]], -sub_delta_present[subj[i],cond_other[i]]);
      }
    } else { // if signal = absent 
      if(choice[i]==1){ // if response = YES (incorrect)
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i],cond_other[i]], sub_delta_absent[subj[i],cond_other[i]]);
      } else { // if response = NO (correct), invert beta and delta
        RT[i] ~ wiener(sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i],cond_other[i]], -sub_delta_absent[subj[i],cond_other[i]]);
      }
    }
  }
}
...
generated quantities {
  ...
  matrix<lower=0,upper=4>[N_groups,N_cond_other] mu_alpha_cond;
  matrix<lower=0,upper=1>[N_groups,N_cond_other] mu_beta_cond;
  ...
  for (i in 1:N_groups){
    for (j in 1:N_cond_other){
      mu_alpha_cond[i,j] = 0.1 + 3.9*Phi(mu_grp_alpha_pr[i]+cond_alpha_pr[i,j]); //start point group mean by emo conditions
      mu_beta_cond[i,j] = Phi(mu_grp_beta_pr[i]+cond_beta_pr[i,j]); //start point group mean by emo conditions
      mu_delta_present_cond[i,j] = -4 + 8*Phi(mu_grp_delta_present_pr[i]+cond_delta_pr[i,j]);//delta group mean, stim present by emo conditions
      mu_delta_absent_cond[i,j] = -4 + 8*Phi(mu_grp_delta_absent_pr[i]+cond_delta_pr[i,j]);//delta group mean, stim absent by emo conditions
    }
  }
  ...
  { // local section. calculate log_lik for each observation
    for (i in 1:N_obs){ 
      if(cond[i]==1){ // if signal = present
        if(choice[i]==1){ // if response = YES (correct)
          log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i],cond_other[i]], sub_delta_present[subj[i],cond_other[i]]);
        } else { // if response = NO (incorrect), invert beta and delta
          log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i],cond_other[i]], -sub_delta_present[subj[i],cond_other[i]]);
        }
      } else{ // if signal = absent 
        if(choice[i]==1){ // if response = YES (incorrect)
          log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], sub_beta[subj[i],cond_other[i]], sub_delta_absent[subj[i],cond_other[i]]);
        } else { // if response = NO (correct), invert beta and delta
          log_lik[i] += wiener_lpdf(RT[i] | sub_alpha[subj[i],cond_other[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i],cond_other[i]], -sub_delta_absent[subj[i],cond_other[i]]);
        }
      }
    }
  }
